...
import grisu.grin.model.Filters;
...
GroupRestrictions {
	...
	public Collection<Package> filterPackages(Application application) {
		...
		return Collections2.filter(getPackages(), Filters.filterResource(application));
	...
	public boolean providesPackage(String app, String version) {
		...
		for ( Package temp : getPackages() ) {
			...
			if (app.equalsIgnoreCase(tempApp)) {
				if ( StringUtils.isBlank(version) || Constants.NO_VERSION_INDICATOR_STRING.equals(tempVersion) ) {
					return true;
				}
